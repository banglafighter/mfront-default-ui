...
import {
    ...
    SharedFieldGroupInput,
    SharedInputGroupAddon
} from "../internal/shared-field-group";
...
function ComboboxClear({ className, ...props }: BasicUICombobox.Clear.Props) {
  return (
    <BasicUICombobox.Clear
      data-tag="combobox-clear"
      render={<SharedFieldGroupButton variant="ghost" size="iconXs" />}
      className={className}
      {...props}
    >
...
function ComboboxChip({ className, children, showRemove = true, ...props }: BasicUICombobox.Chip.Props & { showRemove?: boolean }) {
  return (
    <BasicUICombobox.Chip
      data-tag="combobox-chip"
      className={mergeWind(
        "flex h-[calc(--spacing(5.5))] w-fit items-center justify-center gap-1 rounded-sm bg-muted px-1.5 text-xs font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[tag=combobox-chip-remove]:pr-0",
        className
      )}
      {...props}
    >
    ...
    <BasicUICombobox.Portal>
      ...
      >
        <BasicUICombobox.Popup
          data-tag="combobox-content"
          data-chips={!!anchor}
          className={mergeWind("group/combobox-content relative max-h-(--available-height) w-(--anchor-width) max-w-(--available-width) min-w-[calc(var(--anchor-width)+--spacing(7))] origin-(--transform-origin) overflow-hidden rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 data-[chips=true]:min-w-(--anchor-width) data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 *:data-[tag=field-group]:m-1 *:data-[tag=field-group]:mb-0 *:data-[tag=field-group]:h-8 *:data-[tag=field-group]:border-input/30 *:data-[tag=field-group]:bg-input/30 *:data-[tag=field-group]:shadow-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95", className )}
          {...props}
        />
      </BasicUICombobox.Positioner>
    </BasicUICombobox.Portal>
